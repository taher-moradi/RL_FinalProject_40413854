...
Fixed Bug 3: Default student_id set to "40413854".
...
class TargetEnvironmentBuilder:
    ...
    def build_similar_target(self, shift_ratio: float = 0.18, seed: int = 101) -> Tuple[np.ndarray, Dict[str, Tuple[int, int]]]:
        """Creates Similar Target Environment (15-20% shift, Start/Key/Goal fixed)."""
        attempt_seed = seed
        while True:
    ...
    def build_different_target(self, shift_ratio: float = 0.40, seed: int = 202) -> Tuple[np.ndarray, Dict[str, Tuple[int, int]]]:
        """Creates Different Target Environment (35-40% shift, shifted key, added penalties)."""
        attempt_seed = seed
        while True:
            rng = np.random.default_rng(attempt_seed)
            grid = self.source_grid.copy()
            ...
            if empty_for_key:
                new_key = empty_for_key[int(rng.integers(0, len(empty_for_key)))]
